...
BOOST_AUTO_TEST_SUITE( Conversion );//{
  ...
  BOOST_AUTO_TEST_CASE( to_double_from_cfloat ) {
    BOOST_CHECK_EQUAL( ConvertCoeff<double>( Fc(1.,0) ), 1. );
    BOOST_CHECK_EQUAL( ConvertCoeff<double>( Fc(-1.,0) ), -1. );
    BOOST_CHECK_THROW( ConvertCoeff<double>( Fc(-1.,1) ), runtime::physical::exception );
    BOOST_CHECK_THROW( ConvertCoeff<double>( Fc(0.,1) ), runtime::physical::exception );
  }

  BOOST_AUTO_TEST_CASE( to_float_from_cfloat ) {
    BOOST_CHECK_EQUAL( ConvertCoeff<float>( Fc(1.,0) ), 1.f );
    BOOST_CHECK_EQUAL( ConvertCoeff<float>( Fc(-1.,0) ), -1.f );
    BOOST_CHECK_THROW( ConvertCoeff<float>( Fc(-1.,1) ), runtime::physical::exception );
    BOOST_CHECK_THROW( ConvertCoeff<float>( Fc(0.,1) ), runtime::physical::exception );
  }
  ...
  BOOST_AUTO_TEST_CASE( to_cfloat_from_cdouble ) {
    ...
    BOOST_CHECK_EQUAL( ConvertCoeff<Fc>( Dc(-1.,0) ), -1.f );
    BOOST_CHECK_EQUAL( ConvertCoeff<Fc>( Dc(1.,1) ), Fc(1.f,1) );
    BOOST_CHECK_EQUAL( ConvertCoeff<Fc>( Dc(-1.,1) ), Fc(-1.f,1) );
  }

  BOOST_AUTO_TEST_CASE( to_cdouble_from_cfloat ) {
    BOOST_CHECK_EQUAL( ConvertCoeff<Dc>( Fc(1.,0) ), 1. );
    BOOST_CHECK_EQUAL( ConvertCoeff<Dc>( Fc(-1.,0) ), -1. );
    BOOST_CHECK_EQUAL( ConvertCoeff<Dc>( Fc(1.,1) ), Dc(1.,1.) );
    BOOST_CHECK_EQUAL( ConvertCoeff<Dc>( Fc(-1.,1) ), Dc(-1.,1.) );
  }

  BOOST_AUTO_TEST_CASE( to_cfloat_from_cfloat ) {
    BOOST_CHECK_EQUAL( ConvertCoeff<Fc>( Fc(1.,0) ), 1.f );
    BOOST_CHECK_EQUAL( ConvertCoeff<Fc>( Fc(-1.,0) ), -1.f );
    BOOST_CHECK_EQUAL( ConvertCoeff<Fc>( Fc(1.,1) ), Fc(1.f,1) );
    BOOST_CHECK_EQUAL( ConvertCoeff<Fc>( Fc(-1.,1) ), Fc(-1.f,1) );
  }
